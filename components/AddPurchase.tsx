...
const AddPurchase: React.FC = memo(() => {
  ...
  const [formData, setFormData] = useState({
    ...
    status: editingPurchase?.status || 'نقدي' as any,
    currency: editingPurchase?.currency || 'YER' as Currency,
  ...
  }, [formData, isSubmitting, suppliers, totalAmount, editingPurchase, addPurchase, user, navigate, addNotification, formatValue]);
        ...
        <div className={`p-6 sm:p-8 rounded-[2.5rem] border-2 shadow-2xl space-y-6 ${resolvedTheme === 'dark' ? 'bg-slate-900 border-white/5' : 'bg-white border-slate-100'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className="space-y-1.5">
                ...
                <CurrencySwitcher value={formData.currency} onChange={v => setFormData(p => ({...p, currency: v}))} activeColor="bg-orange-600" />
             </div>
             <div className="space-y-1.5">
                ...
                <div className="bg-slate-50 dark:bg-white/5 p-1 rounded-2xl flex gap-1 border border-slate-100 dark:border-white/5 shadow-inner w-full">
                  {['نقدي', 'آجل'].map(s => (
                    <button key={s} type="button" onClick={() => setFormData(p => ({...p, status: s as any}))} className={`flex-1 py-3 rounded-xl font-black text-[10px] transition-all ${formData.status === s ? (s === 'آجل' ? 'bg-rose-500 text-white' : 'bg-orange-500 text-white shadow-md') : 'text-slate-400'}`}>{s}</button>
                  ))}
                </div>
             </div>
          </div>

          <BaseInput label="ملاحظات الشراء" icon="📝" placeholder="مثلاً: بضاعة درجة أولى..." value={formData.notes} onChange={e => setFormData(p => ({...p, notes: e.target.value}))} />
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-100 dark:border-white/5">
             ...
             <div className="text-center space-y-2 border-r border-slate-100 dark:border-white/5">
                ...
                <input type="number" className="w-full bg-transparent text-center font-black text-5xl outline-none text-orange-500 tabular-nums" value={formData.unit_price} onChange={e => setFormData(p => ({...p, unit_price: e.target.value === '' ? '' : parseFloat(e.target.value)}))} />
             </div>
          </div>
        </div>
